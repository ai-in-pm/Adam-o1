...
from adam-o1.adam-o1_graph import agentic_flow
...
st.set_page_config(
    page_title="Adam-o1 - Agent Builder",
    page_icon="🤖",
    layout="wide",
)

# Set custom theme colors to match Adam-o1 logo (green and pink)
...
# Function to reload the adam-o1_graph module
def reload_adam-o1_graph():
    """Reload the adam-o1_graph module to apply new environment variables"""
    try:
        # First reload pydantic_ai_coder
        import adam-o1.pydantic_ai_coder
        importlib.reload(adam-o1.pydantic_ai_coder)
        
        # Then reload adam-o1_graph which imports pydantic_ai_coder
        import adam-o1.adam-o1_graph
        importlib.reload(adam-o1.adam-o1_graph)
        
        st.success("Successfully reloaded Adam-o1 modules with new environment variables!")
        return True
    except Exception as e:
        st.error(f"Error reloading Adam-o1 modules: {str(e)}")
        return False
...
def generate_mcp_config(ide_type):
    ...
    if ide_type == "Windsurf":
        ...
        return f"{python_path} {server_script_path}", f"docker run --rm -p 8100:8100 adam-o1:latest python mcp_server.py"
    elif ide_type == "Cline":
...
def mcp_tab():
    ...
    if st.session_state.selected_ide:
        ...
        with config_tab2:
            ...
            st.markdown("- Must be running Adam-o1 not within a container")           
...
async def chat_tab():
    """Display the chat interface for talking to Adam-o1"""
...
def intro_tab():
    """Display the introduction and setup guide for Adam-o1"""
    # Display the banner image
    st.image("public/Adam-o1.png", use_container_width=True)
    
    # Welcome message
    st.markdown("""
    # Welcome to Adam-o1!
    
    Adam-o1 is an AI meta-agent designed to autonomously build, refine, and optimize other AI agents.
    
    It serves both as a practical tool for developers and as an educational framework demonstrating the evolution of agentic systems.
    Adam-o1 is developed in iterations, starting with a simple Pydantic AI agent that can build other Pydantic AI agents,
    ...
    st.markdown("Follow these concise steps to get Adam-o1 up and running (IMPORTANT: come back here after each step):")
    ...
    with st.expander("Step 1: Environment Configuration", expanded=True):
        ...
        These settings determine how Adam-o1 connects to external services and which models it uses.
        """)
    ...
    with st.expander("Step 2: Database Setup", expanded=False):
        ...
        Adam-o1 uses Supabase for vector storage and retrieval:
    ...
    with st.expander("Step 5: MCP Configuration (Optional)", expanded=False):
        ...
        This enables you to use Adam-o1 directly from your AI-powered IDE.
        """)
    ...
    # Step 6: Using Adam-o1
    with st.expander("Step 6: Using Adam-o1", expanded=False):
        st.markdown("""
        ### Using Adam-o1
        ...
        3. Adam-o1 will plan and generate the necessary code
        
        You can also use Adam-o1 directly from your AI IDE if you've configured MCP.
        """)
    ...
    - [GitHub Repository](https://github.com/coleam00/adam-o1)
    - [Adam-o1 Community Forum](https://thinktank.ottomator.ai/c/adam-o1/30)
            ...
            with col1:
                ...
                if st.button("Crawl Pydantic AI Docs", key="crawl_pydantic") and not (st.session_state.crawl_tracker and st.session_state.crawl_tracker.is_running):
                    try:
                        ...
                        from adam-o1.crawl_pydantic_ai_docs import start_crawl_with_requests
                ...
                if st.button("Clear Pydantic AI Docs", key="clear_pydantic"):
                    with st.spinner("Clearing existing Pydantic AI docs..."):
                        try:
                            ...
                            from adam-o1.crawl_pydantic_ai_docs import clear_existing_records
...
def database_tab():
    ...
    st.write("Set up and manage your Supabase database tables for Adam-o1.")
...
def agent_service_tab():
    ...
    st.write("Start, restart, and monitor the Adam-o1 agent service for MCP.")
    ...
    with st.container():
        ...
        st.text_area(
            label="Realtime Logs from Adam-o1 Service",
            value=output_text,
...
def environment_tab():
    ...
    st.write("- Configure your environment variables for Adam-o1. These settings will be saved and used for future sessions.")
...
async def main():
    ...
    with st.sidebar:
        st.image("public/Adam-o1LightGrey.png", width=1000)
